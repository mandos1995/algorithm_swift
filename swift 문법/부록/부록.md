...
스위프트 표준 라이브러리에서 알아두면 유용한 타입을 모아 소개한다.

### I.1 Codable

Codable은 스위프트 4 버전에서 처음 소개된 것으로 스위프트의 문법 형식 자체는 아니지만, 여러모로 유용한 타입이기에 소개한다. Codable에 정의를 살펴보면 public typealias Codable = Decodable & Encodable로, Decodable 프로토콜과 Encodable 프로토콜의 조합임을 알 수 있다.

Encodable과 Decodable은 사용자 정의 데이터 타입을 다른 형식으로 쉽게 인코딩하고 디코딩할 수 있는 청사진을 제시하는 프로토콜이다.

사실 Encodable과 Decodable 프로토콜을 준수한다고 해서 그 자체만으로 큰 역할을 하지는 않지만 다른 기능과 결합하면 큰 시너지 효과를 발휘한다. 특히 애플에서 제공하는 **Foundation** 프레임워크 의 여러 클래스와 호환된다.

|Foundation 프레임워크의 클래스|효과|
|---|---|
|JSONEncoder, JSONDecoder|Codable 프로토콜을 준수하는 타입의 데이터를 JSON 문자열로 변환하거나 JSON 문자열을 타입의 인스턴스로 변환할 수 있다.
|NSKeyedArchiver,NSKeyedUnarchiver|Codable 프로토콜을 준수하는 타입의 데이터를 JSON 문자열로 변환하거나 JSON 문자열을 타입의 인스턴스로 변환할 수 있다.
|PropertyListEncoder, PropertyListDecoder|Codable 프로퍼티를 준수하는 타입의 데이터를 프로퍼티 리스트 데이터로 변환하거나 프로퍼티 리스트 데이터를 타입의 인스턴스로 변환할 수 있다.|

위 클래스중 JSONEncoder와 JSONDecoder를 활용하는 방법에 대해 다음 코드를 통해 알아보자.
...
let minseok = Person(name: "minseok", age: 20, gender: .male, friends: [])
let hana = Person(name: "hana", age: 22, gender: .female, friends: [minseok])
let eric = Person(name: "eric", age: 25, gender: .male, friends: [minseok, hana])
...
/*
 {"age":25,"gender":"male","friends":[{"age":20,"gender":"male","friends":[],"name":"minseok"},
 {"age":22,"gender":"female","friends":[{"age":20,"gender":"male","friends":[],"name":"minseok"}],"name":"hana"}],"name":"eric"}
 */
...
위 코드의 Gender 타입에도 Codable을 준수하도록 한 것은, Person 타입이 Codable을 준수하더라도 중첩 타입인 Gender가 Codable을 준수하지 않으면 gender 프로퍼티를 인코딩 혹은 디코딩 할 수 없기 떄문이다. 이 점에 주의해야 한다.

또 한가지 생각해 볼 점이 있는데 바로 JSON 키의 이름 짓는 방법이다. 스위프트는 소문자 카멜케이스로 프로퍼티의 이름을 짓는다. 그런데 다른 언어를 사용하는 서버에서 JSON 객체를 보내는 경우에는 키의 이름이 카멜케이스가 아닌 경우가 있다. 그럴때는 JSONDecoder의 KeyDecodingStrategy와 JSONEncoder의 KeyEncodingStrategy를 활용하면 카멜케이스와 스네이크 케이스를 상호 변환하여 준다.
...
JSONDecoder에는 KeyDecodingStrategy 외에도 DataDecodingStrategy 등도 있고, JSONEncoder 에는 DataEncodingStrategy, KeyEncodingStrategy 등도 있으니 한번 살펴보자.

또, CodingKey 프로토콜을 활용하면 JSON 객체의 키 값과 프로퍼티의 이름이 서로 달라도 값을 매칭해줄 수 있다. Codable 프로토콜을 준수하는 타입의 중첩 타입으로 String 타입의 원시값을 가지며 CodingKey 프로토콜을 준수하는 CodingKeys라는 이름의 열거형을 선언하면 된다. CodingKeys는 해당 타입의 프로퍼티 이름을 모두 케이스로 갖고 있어야 한다.
...
연관값이 없는 열거형인 경우 CaseIterable 프로토콜을 준수하면 각각의 case를 순회할 수 있다. 순회할 case는 allCases 콜렉션으로 제공한다. 또, allCases로 뽑아낸 case 목록 중 randomElement() 메서드로 임의의 case를 만들 수 있다. 의외로 자주 쓸 수 있는 조합이다.
...
성공 혹은 실패에 대한 정보를 담는 타입이다. 제네릭 열거형으로 구현되어 있다. 값이 있음 혹은 없음을 구현한 옵셔널과 유사하지만 옵셔널은 값이 없음에 연관 값이 없는 반면 Result 타입은 실패에도 오류에 대한 정보를 담은 연관 값이 존재한다.
...
print(ascii("ㄱ")) // failure(algorithm_swift.ConvertError.notAnASCII)
```